 ...
 *
 * key 只经环境变量注入（DEEPSEEK_API_KEY 恒需；--judge 另需 JUDGE_API_KEY 或旧名
 * OPENAI_API_KEY）；本机注入首选仓库根 .env.local（已被 gitignore，启动时自动装载；
 * 已存在的环境变量优先不覆盖）。接入点可经 DEEPSEEK_URL / JUDGE_URL（旧名
 * OPENAI_URL 兼容）环境变量覆盖（自定义 OpenAI 兼容网关，尾斜杠自动归一）；
